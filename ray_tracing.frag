...
void initScene() 
{
    ...
    materials[1].kd = vec3(0.5, 0.5, 0.5);
...
Hit hitSphere(const Ray r, const Sphere s) 
{
    ...
    float B = -2 * dot( s.ori - r.ori, r.dir);
...
vec3 shadingPhong(Light light, int matId, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    /* your implementation starts */
    vec3 tex_color = sampleDiffuse(matId, p);
	vec3 ambient = ka * light.Ia; 

    vec3 direction = normalize(s - p);
    float lightPerArea = dot(n, direction);
    float num = max(0.0, lightPerArea);
    vec3 diff_color = kd * light.Id * num * tex_color;

    vec3 reflection = reflect(-direction, n);
    vec3 view = normalize(e - p);
    float num1 = max(0, dot(view, reflection));
    float specularIntensity = pow(num1, shininess);
    vec3 specular = ks * light.Is * specularIntensity;
    
    vec3 phong = ambient + diff_color + specular;
    color = phong;
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0) {		
        ...
        color = tex_color * mat_color;
		/* your implementation ends */

    } else if (matId == 1) {
        p = p - spheres[0].ori;
        float u = 0.5 + atan(p.x, p.z) / (2.0 * M_PI);
        float v = 0.5 - asin(p.y / spheres[0].r) / M_PI;
        vec2 uv = vec2(u, v);
        vec3 texture_color = texture(water_color, uv).rgb;
        color = texture_color * mat_color;

    } else if (matId == 2) {
        p = p - spheres[1].ori;
        float u = 0.5 + atan(p.x, p.z) / (2.0 * M_PI);
        float v = 0.5 - asin(p.y / spheres[1].r) / M_PI;
        vec2 uv = vec2(u, v);
        vec3 texture_color = texture(fire_color, uv).rgb;
        color = texture_color * mat_color;

    } else if (matId == 3) {
        p = p - spheres[2].ori;
        float u = 0.5 + atan(p.x, p.z) / (2.0 * M_PI);
        float v = 0.5 - asin(p.y / spheres[2].r) / M_PI;
        vec2 uv = vec2(u, v);
        vec3 texture_color = texture(rock_color, uv).rgb;
        color = texture_color * mat_color;
    }
...
bool isShadowed(Light light, Hit h) 
{
    ...
    /* your implementation starts */
	Ray shadow_ray = Ray(intersect + Epsilon, dir);
    Hit shadow_hit = findHit(shadow_ray);
    float t = shadow_hit.t;
    if (t > 0.0 && t < t_max) {
        shadowed = true;
    }
	/* your implementation ends */
